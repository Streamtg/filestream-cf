...
const ERROR_409 = {"ok":false,"error_code":409,"description":"Bad Request: file size exceeds limit"};
...
async function onMessage(event, message) {
    ...
    if (message.text && message.text.startsWith("/start ")) {
        ...
        try {
            ...
            return await Bot.sendMessage(message.chat.id, message.message_id, ERROR_407.description);
        }
    ...
    if (message.document) {
        ...
        const buttons = [[{ text: "Mi blog", url: "https://yoelmod.blogspot.com" }]];
        return Bot.sendMessage(message.chat.id, message.message_id, "Send me any file/video/gif/audio *(Telegram<=4GB, Streaming<=20MB)*.", buttons);
    }
